...
class ResBlock_no_ieta_iphi(nn.Module):

    def __init__(self, in_channels, out_channels):
        super(ResBlock_no_ieta_iphi, self).__init__()
        self.downsample = out_channels//in_channels
        self.conv1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=self.downsample, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
...
class ResNet_no_ieta_iphi(nn.Module):

    def __init__(self, in_channels, nblocks, fmaps):
        super(ResNet_no_ieta_iphi, self).__init__()
        self.fmaps = fmaps
        self.nblocks = nblocks
    ...
    def block_layers(self, nblocks, fmaps):
        layers = []
        for _ in range(nblocks):
            layers.append(ResBlock_no_ieta_iphi(fmaps[0], fmaps[1]))
        return nn.Sequential(*layers)
    ...
    def forward(self, X):
        ...
        x = self.fc(x)
        return x
# model = ResNet_no_ieta_iphi(in_channels=len(indices), nblocks=3, fmaps=[8,16,32,64])